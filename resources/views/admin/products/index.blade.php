@php
    ...
    $activeProducts = collect($products)->where('is_active', 1)->count();
    $inactiveProducts = collect($products)->where('is_active', 0)->count();
    $totalValue = collect($products)->sum('price');
@endphp
    ...
    <main class="main">
        <header class="topbar">
            <div class="top-left">
                ...
                <div class="page-title">
                    <h1>Products</h1>
                    <p>Kelola produk investasi, harga, profit, durasi, status aktif, dan minimal VIP.</p>
                </div>
            </div>
            ...
            <div class="top-center">
                <div class="global-search">
                    <span>⌕</span>
                    <input type="text" placeholder="Search product / category..." oninput="syncMainSearch(this.value)">
                </div>
            </div>
            ...
            <div class="top-actions">
                ...
                <a href="/admin/products/create" class="icon-btn" title="Tambah Produk">＋</a>
        ...
        <section class="hero">
            <div class="hero-content">
                ...
                <h2>Atur produk investasi dengan tampilan admin yang lebih rapi.</h2>

                <p>
                    Pantau seluruh produk, kategori, harga, minimal VIP, dan status aktif/nonaktif
                    dalam satu halaman yang clean dan mudah dipakai.
                </p>

                <div class="hero-actions">
                    <a href="/admin/products/create" class="hero-link">+ Tambah Produk</a>
        ...
        <section class="stats">
            <div class="stat-card">
                ...
                <div class="stat-note">Seluruh produk di sistem</div>
            </div>
            ...
            <div class="stat-card green">
                ...
                <div class="stat-note">Produk yang sedang tampil</div>
            </div>
            ...
            <div class="stat-card yellow">
                ...
                <div class="stat-note">Produk nonaktif / disembunyikan</div>
            </div>

            <div class="stat-card purple">
                <div class="stat-top">
                    <div>
                        <div class="stat-label">Total Product Value</div>
                        <div class="stat-value">{{ adminMoney($totalValue) }}</div>
                    </div>
                    <div class="stat-icon">💰</div>
                </div>
                <div class="stat-note">Akumulasi nominal harga produk</div>
            </div>
        </section>
        ...
        <section class="panel">
            <div class="panel-head">
                <div class="panel-title">
                    <b>Products List</b>
                    <span>Daftar produk investasi berdasarkan kategori, harga, VIP, dan status.</span>
                </div>
            ...
            <div class="filters">
                <div class="field">
                    <span>⌕</span>
                    <input id="q" type="text" placeholder="Cari produk / kategori..." oninput="filterProducts()" />
                </div>
                ...
                <div class="field">
                    <select id="vipFilter" onchange="filterProducts()">
            ...
            <div class="meta-row">
                <div class="chips">
                    <span class="chip" id="countChip">Total: {{ $totalProducts }}</span>
                    <span class="chip green">Aktif: {{ $activeProducts }}</span>
                    <span class="chip red">Nonaktif: {{ $inactiveProducts }}</span>
                </div>

                <div id="shownInfo">Menampilkan: {{ $totalProducts }} produk</div>
            </div>
                ...
                <table id="productsTable">
                    ...
                    @forelse($products as $p)
                        @php
                            ...
                            $statusKey = $p->is_active ? 'active' : 'inactive';
                        @endphp
                        ...
                        <tr
                            ...
                            data-vip="{{ (int)($p->min_vip_level ?? 0) }}"
                        >
                            <td data-label="Nama">
                            ...
                            <td data-label="Harga">
                                <span class="money">{{ adminMoney($p->price ?? 0) }}</span>
                            </td>

                            <td data-label="VIP Min">
                                <span class="badge vip">VIP {{ (int)($p->min_vip_level ?? 0) }}</span>
                            </td>
                            ...
                            <td data-label="Aksi">
                                <div class="actions-col">
                                    <a href="/admin/products/{{ $p->id }}/edit" class="action-link">Edit</a>

                                    <form method="POST" action="/admin/products/{{ $p->id }}/toggle" style="display:inline">
                                        @csrf

                                        <button
                                            class="toggle-btn {{ $p->is_active ? 'off' : 'on' }}"
                                            type="submit"
                                            onclick="return confirm('{{ $p->is_active ? 'Nonaktifkan produk ini?' : 'Aktifkan produk ini?' }}')"
                                        >
...
<script>
    ...
    function filterProducts() {
        ...
        const rows = document.querySelectorAll('#productsBody tr[data-name]');
        let shown = 0;
        ...
        rows.forEach((tr) => {
            ...
            const matchQ = !q || name.includes(q) || category.includes(q);
            const matchStatus = !status || s === status;
            const matchVip = !vip || v === vip;

            const ok = matchQ && matchStatus && matchVip;
        ...
        if (shownInfo) shownInfo.textContent = `Menampilkan: ${shown} produk`;
